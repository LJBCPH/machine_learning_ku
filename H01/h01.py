...
markov_bound = np.minimum(1/(alphas*2),1)
chebyshevs_bound = np.minimum(1/(80*(alphas - 0.1)**2), 1)
...
def plot_knn(errors, title = None, n = 10, err_plot = False):
    fig = plt.figure()
...
def run_knn(n, data, labels = labels,plot = True):
    digits_estimated = []
    validation_errors = []
    error_var = []
    train = data[:100,]
    ...
    if plot == True:
        plot_knn(validation_errors, 
                 title = 'C:/Users/lucas/OneDrive/Skrivebord/repo/machine_learning_ku/H01/plots/knn_'+ f'{digits=}'.split('=')[0] + '_n_' + str(n) + '.png',
                 n = n)
    
    
    for i in range(50):
        error_var.append(np.var(validation_errors[0:250:(i+50)]))
...
n = 10
x_val1, y_val1 = digits[100+1*n:100+(2*n),], labels[100+1*n:100+(2*n)]
x_val2, y_val2 = digits[100+2*n:100+(3*n),], labels[100+2*n:100+(3*n)]
x_val3, y_val3 = digits[100+3*n:100+(4*n),], labels[100+3*n:100+(4*n)]
x_val4, y_val4 = digits[100+4*n:100+(5*n),], labels[100+4*n:100+(5*n)]
x_val5, y_val5 = digits[100+5*n:100+(6*n),], labels[100+5*n:100+(6*n)]
